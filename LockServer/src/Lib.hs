...
startApp = withLogging $ \ aplogger -> do
  ...
  let settings = setPort (read lockServerPort ) $ setLogger aplogger defaultSettings
...
server = enableLock
    ...
    where 
      ...
      enableLock :: FileInformation -> Handler Bool
      enableLock (FileInformation s f) = liftIO $ do
        
        -- no need to check if file is locked 
        -- as the client should always check it with isLocked method before this request 
      ...
      disableLock :: FileInformation -> Handler Bool
      disableLock (FileInformation s f) = liftIO $ do
        
        deleteLock s f
        
        return True

      isLocked :: FileInformation -> Handler Bool
      isLocked (FileInformation s f) = liftIO $ do

        querried_list <- findLock s f             -- querry rows with file_name
        return $ hasLock querried_list            -- return if it's empty( file exist? )
          


-- findLock :: String -> String -> [LockItem]
findLock s f = do
  withMongoDbConnection $ do
    docs <- find (select ["server" =: s, "file" =: f] "LOCKING_RECORD") >>= drainCursor
    -- return $ catMaybes $ DL.map (\ b -> fromBSON b :: Maybe LockItem) docs
    return docs

-- addLock :: String -> String -> IO ()
addLock s f = do
  let locking_record = ["server" =: s, "file" =: f]
  withMongoDbConnection $ insert ( "LOCKING_RECORD" ) locking_record 

-- delete locks
deleteLock s f = withMongoDbConnection $ delete ( select ["server" =: s, "file" =: f] "LOCKING_RECORD" )

-- hasLock :: [LockItem] -> Bool
-- if querried locklist is empty then false else true
hasLock [] = False
hasLock _ = True
